...
engine::Surface::Surface(const std::shared_ptr<engine::EngineState>& engineState,
                         const std::string& title, int width, int height)
    : engineState_(engineState), title_(title), width_(width), height_(height) {}

void engine::Surface::onStartup() {
    engineState_->window = sf::RenderWindow(
